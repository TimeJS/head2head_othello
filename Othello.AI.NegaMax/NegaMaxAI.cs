...
public class NegaMaxAI : IOthelloAI
{
    ...
    public NegaMaxAI(int maxDepth = 4)
    {
    ...
    {
        // Run search off the caller thread so UI/build loops don't block
    ...
    private int Negamax(BoardState board, int depth, int alpha, int beta, DiscColor toMove, DiscColor rootColor, CancellationToken ct)
    {
        ...
        {
            // terminal or depth reached: evaluate from rootColor perspective
    ...
    {
        // Simple heuristic: weighted piece count + corner bonus
        int score = 0;
        int[,] weights = {
    ...
    // Reuse same helpers from your Random AI files
